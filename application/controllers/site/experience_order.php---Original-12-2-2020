...
class Experience_Order extends MY_Controller { 
	...
	function __construct(){
		...
		if($_SESSION['sMainCategories'] == ''){
			...
			$_SESSION['sMainCategories'] = $this->experience_order_model->get_all_details(CATEGORY,array('rootID'=>'0','status'=>'Active'),$sortArr);
		...
		if($_SESSION['sColorLists'] == ''){
			...
			$_SESSION['sColorLists'] = $this->experience_order_model->get_all_details(LIST_VALUES,array('list_id'=>'1'));
	...
	public function index(){
		...
		if($this->uri->segment(2) == 'success'){
			...
			$PaymentSuccessCheck = $this->experience_order_model->get_all_details(EXPERIENCE_BOOKING_PAYMENT,array('user_id' => $UserNo, 'dealCodeNumber' => $DealCodeNo,'status'=>'Paid'));
		...
		//echo "<pre>";print_r($EnquiryUpdate->result());die;	
		 ...
		 // No coupon for experience purchase
		 ...
		 $this->experience_order_model->update_details(EXPERIENCE_BOOKING_PAYMENT,$data,array('dealCodeNumber'=>$DealCodeNo));

		// $data = array('totalAmt' => $coupon['2']);

		 //$this->experience_order_model->update_details(RENTALENQUIRY,$data,array('id'=>$eInq_id));

		 

		 $data = array('code' => trim($coupon['0']));

		 $couponi = $this->experience_order_model->get_all_details(COUPONCARDS,$data);
		 ...
		 $this->experience_order_model->update_details(COUPONCARDS,$data,array('id'=>$couponi->row()->id));
		...
		*/
		...
		//$this->experience_order_model->commonInsertUpdate(RENTALENQUIRY,'update',$excludeArr1,$dataArr1,$condition1);					

								

		//$this->data['Confirmation'] = $this->experience_order_model->PaymentSuccess($this->uri->segment(3),$this->uri->segment(4),$transId,$Pray_Email);
...
//print_r($this->data['Confirmation']->result());		
		...
		//Experience is not based on single booking per date so calendar update is not needed
		...
		*/						
		...
		$condition = array('status' => 'Active', 'seo_tag'=>'experience-host-fee');
...

		{
			...
			// rep_code not applicable to experience
		...
		{}*/
			...
			'exp_cancel_percentage'=>$orderDetails->row()->exp_cancel_percentage,
...
public function booking_conform_mail($paymentid){
	...
	$proImages=base_url().EXPERIENCEPATH.$RentalPhoto->row()->product_image;
...
//$currency_amount=$currency_price*$noofnights;
...
$currency_netamount = $currency_amount + $securityDeposite + $currency_serviceFee + $cleaningFee;
...
public function mail_owner_admin_booking($got_values)
			...
			{//print_r($got_values);die;
					...
					$this->experience_order_model->simple_insert(INBOX,array('sender_id'=>$this->session->userdata('ContacterEmail'),'user_id'=>$sender_email,'mailsubject'=>$template_values['news_subject'],'description'=>stripslashes($message)));
					...
					$email_send_to_common1 = $this->experience_order_model->common_email_send($email_values2);*/
				...
				if ($productUserDetails->num_rows()==1){
					...
					if (in_array('comments', $emailNoty)){
						...
						if ($commentDetails->num_rows() == 1){
							...
							$this->experience_order_model->simple_insert(INBOX,array('user_id'=>$productUserDetails->row()->email,'sender_id'=>$sender_email,'mailsubject'=>$subject,'description'=>stripslashes($message)));
