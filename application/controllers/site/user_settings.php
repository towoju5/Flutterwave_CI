    ...
    public function update_profile(){
    	...
    	if ($this->checkLogin('U') == ''){
	    	...
	    	if ($update == '1'){
				...
				if($this->lang->line('Done ! Your profile looks even better now') != '') 
				{ 
					$message = stripslashes($this->lang->line('Done ! Your profile looks even better now')); 
				} 
				else 
				{
					$message = "Done ! Your profile looks even better now";
				}
...
//	    	$this->data['purchasesList'] = $this->user_model->get_group_gifts_list($this->checkLogin('U'));
	...
	public function user_profile() {
		...
		/* Sathyaseelan 
		$this->data['ReviewDetails']=$this->review_model->get_productreview_aboutyou1($this->data['loginCheck']);
		*/
		// echo $this->data['loginCheck'];
		// echo $this->db->last_query(); die;
        ...
        $this->data['WishListCat'] = $this->product_model->get_list_details_wishlist($userid);
		   ...
		   /**Wish list for Properties**/
			...
			/**Wish list for Properties End **/
			
			
			/**Wish list for Experiences**/
    ...
    public function change_photo(){
    	if ($this->checkLogin('U')==''){
			...
			public function update_languages()
			{
				...
				{
					$returnStr .= "<li id='".$lang->language_code."'>".$lang->language_name."<small><a class='text-normal' href='javascript:void(0);' onclick='delete_languages(this,'".$lang->language_code."')>x</a></small></li>";
				}
