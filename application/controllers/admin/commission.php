...
class Commission extends MY_Controller {
	...
	public function display_commission_representative_lists()
	{
			...
			//$this->data['representativelists'] = $this->commission_model->get_all_rep_details('fc_commission_tracking');
	...
	public function display_commission_tracking_lists(){
		...
		if ($this->checkLogin('A') == ''){
			...
			if($rep_code!=''){
				$condition = array('group'=>'Seller','status'=>'Active','rep_code'=>$rep_code);
			}else{	
				$condition = array('group'=>'Seller','status'=>'Active');
			}
			
			$sellerDetails = $this->commission_model->get_all_details(USERS,$condition);
				...
				foreach($rental_booking_details[$sellerEmail] as $rentals)
				{
					...
					if(!empty($payment)){
						...
						if($payment->is_coupon_used=='Yes'){
							...
							$this->data['trackingDetails'][$sellerEmail]['coupon_discount']=($this->data['trackingDetails'][$sellerEmail]['coupon_discount']+$coupon_discount);
						}
					}
					...
					$cancel_amountBf = $rentals['subtotal']/100 * $rentals['cancel_percentage'];
					$cancel_amountPayableAmt = $rentals['payable_amount']/100 * $rentals['cancel_percentage']; //for payable amount its already converted in above lines
					if($rentals['currencycode'] != $this->data['admin_currency_code']){	
					...
					/* 	if ($rentals['cancel_percentage']!='100') { 
							...
							$cancel_amount = $rentals['payable_amount']/100 * $rentals['cancel_percentage'];
					...
					/**to show AmountTohost if the property is cancelled **/
					if($rentals['cancelled'] =='Yes'){
						
						//check policy here
						if ($rentals['cancel_percentage']!='0') {    //can_percentage 0 means no cash back to guest
					...
					/**to show cancelAmount if the property is cancelled **/
					
					
					
					
					if($rentals['cancelled'] =='Yes'){
						$this->data['trackingDetails'][$sellerEmail]['cancel_percentage'] += $cancel_amount;
					}else{	
			...
			if(count($paidAmountQry) != 0){ 
				...
				foreach($paidAmountQry as $rental_paid){
				$this->data['trackingDetails'][$sellerEmail]['paid'] = $this->data['trackingDetails'][$sellerEmail]['paid'] + $rental_paid['payable_amount'];	
				}
				...
				if(count($paidAmountQry) != 0 ){ 
					...
					/** Start - Convert Cancel Amount **/
						$cancel_amountBf = $rental_paid['subtotal']/100 * $rental_paid['cancel_percentage'];
						$cancel_amountPayableAmt = $rental_paid['payable_amount']/100 * $rental_paid['cancel_percentage']; 												
						if($rental_paid['currencycode'] != $admin_currencyCode){	
							if ($rental_paid['cancel_percentage']!='100') { 							
								$cancel_amount=customised_currency_conversion($currencyPerUnitSeller,$cancel_amountBf); 
							}else{							
								//$cancel_amount=$cancel_amountPayableAmt; 	
					...
					if($rental_paid['currencycode'] != $admin_currencyCode){	
						$payable=customised_currency_conversion($currencyPerUnitSeller,$payableDb); 
					}else{
					...
					/* 	
						...
						if($rental_paid['currencycode']!=$admin_currencyCode){
							...
							$re_paid=customised_currency_conversion($currencyPerUnitSeller,$rental_paid['payable_amount']) - $cancel_amount;
						}else{
			...
			//$this->data['trackingDetails'] = $this->commission_model->get_all_commission_tracking($sellerEmail);
			$this->load->view('admin/commission/display_tracking_lists',$this->data);
	...
	public function display_product_list($email){
		...
		if ($this->checkLogin('A') == ''){
			...
			$this->data['heading'] = 'Product wise payment details';
	...
	public function display_wallet_payments_list(){
		...
		if ($this->checkLogin('A') == ''){
			...
			$rep_code=ltrim($this->session->userdata('fc_session_admin_rep_code'), '0'); 
			if($rep_code!=''){
				$condition = array('status'=>'Active','rep_code'=>$rep_code);
			}else{	
		...
		//echo "<pre>";	print_r($sellerDetails->result()); die; $this->data['trackingDetails']
...

			{
				...
				{	
                    ...
                    {
						//$usedWallet = convertCurrency($currency_code ,$this->data['admin_currency_code'],$usedWallet);	
...
//echo $this->data['trackingDetails'][$sellerEmail]['payable_amount'];
...

				{
					...
					//echo $this->data['trackingDetails'][$sellerEmail]['id'];
...
//echo $this->data['trackingDetails'][$sellerEmail]['total_amount'];
//echo '<br>';
//echo AdminCurrencyValue($rentals['prd_id'],$rentals['total_amount']);
					//$this->data['trackingDetails'][$sellerEmail]['total_amount'] = AdminCurrencyValue($rentals['prd_id'],$rentals['total_amount']);
//echo $this->data['trackingDetails'][$sellerEmail]['total_amount']; echo '</br>';
		...
		if ($this->checkLogin('A') == ''){
			...
			if(count($paidAmountQry) != 0 ){ 
				foreach($paidAmountQry as $rental_paid)
				{
					...
					if($rental_paid['currencycode']!=$admin_currencyCode){//$re_paid=customised_currency_conversion($currencyPerUnitSeller,$rental_paid['payable_amount']);
...

	{
		 ...
		 /*Paypal integration start */
          ...
          //  $totalAmount = $totalAmount * $usdvalue->row()->currency_rate;
			...
			//print_r($paypal_settings);exit;

			$lastFeatureInsertId ='1';
           ...
           // $this->paypal_class->add_field('hostEmail',$hostEmail); // host email

			

			

			$this->paypal_class->add_field('return',$cancel_url); // Return URL
			...
			$this->paypal_class->add_field('notify_url', $cancel_url); // Notify url
			...
			$this->paypal_class->add_field('user_id', '1');
			...
			//echo $totalAmount;die;
			 ...
			 // $this->paypal_class->add_field('amount', $totalAmount); // Price
			  ...
			  $this->paypal_class->add_field('amount', '1');
		...
		foreach($theCheckIn as $date){
			...
			if($checkinBeforeDay <= $current_date){
				 foreach($thebookingNo as $b_no){
					$this->commission_model->update_details(COMMISSION_TRACKING, array('paid_status'=>'yes'), array('booking_no'=>$b_no));
				 }	 
			}
