	...
	public function order_review(){
		...
		if ($this->checkLogin('A') == ''){
			...
			//$order_details = $this->order_model->get_all_details(PAYMENT,array('dealCodeNumber'=>$dealCode,'status'=>'Paid'));
				...
				$this->db->select('p.*,pAr.attr_name,pr.product_title,rq.Bookingno,rq.secDeposit,rq.checkin,rq.NoofGuest,rq.subTotal,rq.serviceFee,rq.checkout,rq.numofdates,rq.currencyPerUnitSeller,pi.product_image,ct.host_fee');
				$this->db->from(PAYMENT.' as p');
				$this->db->join(PRODUCT_ATTRIBUTE.' as pAr' , 'pAr.id = p.attribute_values','left');
				$this->db->join(RENTALENQUIRY.' as rq' , 'rq.id = p.EnquiryId','left');
				$this->db->join(PRODUCT.' as pr' , 'pr.id = p.product_id','left');
				$this->db->join(COMMISSION_TRACKING.' as ct' ,'ct.booking_no = rq.Bookingno','left');
