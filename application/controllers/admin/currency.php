...
class Currency extends MY_Controller {
		...
		if ($this->checkLogin('A') == ''){
			...
			if ($currency_id == ''){
                ...
                if($currency_check->num_rows() >0){
                	// echo '<pre>'.print_r($currency_check->result());die;
				  $currency = array();
				  ...
				  // print_r($_POST); die;
			        if(in_array($this->input->post('currency_type'),$currency) OR in_array($this->input->post('country_name'),$currency_country) OR in_array($this->input->post('currency_symbols'),$currency_symbol)){
	...
	public function get_currency_code_and_symbol()
	{
		...
		// print_r($country_det);die;
		// echo $currency_symbols["INR"]["symbol"];die;
		$currency_code= $country_det->{currencies}[0];
		// if(in_array(strtoupper($currency_code), $currency_symbols))
			$currency_symbol= $currency_symbols[strtoupper($currency_code)]["symbol"];
		// else
		// 	$currency_symbol="";

		// foreach($currency_symbols as $code=>$symbol)
		// {
		// 	if($code==strtoupper($currency_code))
		// 	{
		// 		$currency_symbol= $currency_symbols[$code]["symbol"];
		// 		break;
		// 	}
		// 	else
		// 	{
