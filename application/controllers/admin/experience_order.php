...
class Experience_Order extends MY_Controller {
	...
	public function display_order_paid(){
		if ($this->checkLogin('A') == ''){
			...
			$this->data['heading'] = 'Paid payment list';
	...
	public function order_review(){
		...
		if ($this->checkLogin('A') == ''){
			...
			//$order_details = $this->experience_order_model->get_all_details(PAYMENT,array('dealCodeNumber'=>$dealCode,'status'=>'Paid'));
			
				$this->db->select('p.*,pr.experience_title as product_title,rq.Bookingno,rq.checkin,rq.NoofGuest,rq.subTotal,rq.serviceFee,rq.secDeposit,rq.checkout,rq.numofdates,rq.currencyPerUnitSeller,rq.currencycode,pi.product_image');
			...
			if ($order_details->num_rows()==0){
				...
				$this->data['order_details'] = $order_details;
				//echo $this->db->last_query();
				//echo "<pre>"; print_r($order_details->result()); die;
