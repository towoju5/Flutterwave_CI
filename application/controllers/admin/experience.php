...
class Experience extends MY_Controller {
    ...
    /**
     * 
     * This function loads the experience list page
     */
	...
	/**
	 * 
	 * This function all the experience type record from db
	 */
	public function experienceTypeList(){
		if ($this->checkLogin('A') == ''){
			redirect('admin');
		}else {
			$this->data['heading'] = 'Category Type Details';
			$this->data['experienceList'] = $this->experience_model->view_experienceType_details();
	...
	public function addExperienceType_from(){
		if ($this->checkLogin('A') == ''){
			...
			$this->data['heading'] = 'Add Category Type ';
			$this->load->view('admin/experience/add_experience_type',$this->data);
		}
	}

	/* save new experience type starts */
	public function saveExperienceType(){
		if ($this->checkLogin('A') == ''){
			...
			$duplicate_name = $this->experience_model->get_all_details(EXPERIENCE_TYPE,$check);
				//echo $duplicate_name->num_rows();die;
	...
	/* Experience type status change ends */
	...
	public function experienceList(){
		...
		if ($this->checkLogin('A') == ''){
			...
			$this->data['heading'] = 'Experience Details';
			$this->data['experienceList'] = $this->experience_model->get_all_details(EXPERIENCE,array());
		...
		//echo 'gangatharan';die;
			...
			//$this->data['productList'] = $this->experience_model->view_product_details($condition);
			
			//echo "<pre>"; print_r($this->data['productList']->result()); die;
			$this->data['experienceList'] = $this->experience_model->get_allthe_details($_GET['status'],$_GET['city'],$_GET['id']);
			//echo $this->db->last_query();
			//print_r($this->data['experienceList']->result());
			...
			//echo $this->db->last_query();
			//print_r($this->data['experienceList']);exit;
...
$firstname = $_POST['firstname'];
	...
	/* add and edit experience ends */	
	
	/* New - add and edit experience starts */

	public function add_experience_form_new($id=''){
		
		//print_r($this->session->userdata('fc_session_admin_privileges'));
		
		

		//if ($this->data['allPrev'] == '1' || in_array('1', $Experience)){
	...
	public function get_new_timesheet(){
		if ($this->checkLogin('A') != ''){
			...
			/*$str.='<input class="dev_time input_class" id="end_time_'.$exp_dates_id.'" name="end_time" type="hidden" value="" style="width:40%;" onkeypress="return return_false_fun(event)"';
			$str.=($same_day==1) ? "readOnly" : "" ;
			
			$str.=' >';*/
			
			$str.='<input class="dev_time input_class" id="end_time_'.$exp_dates_id.'" name="end_time1" type="text" value="" style="width:40%;" onkeypress="return return_false_fun(event)"';
			$str.=($same_day==1) ? "readOnly" : "" ;
			
			$str.=' >';
			$str.='</p>';
			
			$str.='<p>Title <span class="req"><small>*</small></span> <input name="title" type="text" value="" style="width:100%;" id="title_'.$exp_dates_id.'" class="input_class" maxlength="20" onkeyup="char_count(this)"><span class="small_label"><span id="title_'.$exp_dates_id.'_char_count">20</span> characters remaining</span></p>';
			
			$str.='<p>Description <span class="req"><small>*</small></span><textarea maxlength="150" name="description" id="description_'.$exp_dates_id.'" onkeyup="char_count(this)"></textarea><span class="small_label"><span id="description_'.$exp_dates_id.'_char_count">150</span> characters remaining</span></p>';
	...
	public function get_timesheets_for_edit(){
		if ($this->checkLogin('A') != ''){
			...
			foreach($data as $time){
				...
				$str.='<p>Title <span class="req"><small>*</small></span> <input name="title" maxlength="20" onkeyup="char_count(this)" id="update_title_'.$time->id.'" type="text" value="'.$time->title.'" style="width:100%;" class="input_class"><span class="small_label"><span id="update_title_'.$time->id.'_char_count">';
	...
	public function UpdateExperience(){ //echo 'ganbas';die;
		/*image upload end */
	...
	public function InsertProductImage1($prd_id) {
		...
		{
			$uploaddir = "server/php/experience/";	//a directory inside
			$uploaddir_resize = "server/php/experience/resize/";
			...
			foreach ($_FILES['files']['name'] as $name => $value)
			{
				...
				{
						/*compress and Resize*/ 
						$source_photo = 'server/php/experience/'.$image_name.'';
						$dest_photo = $newname;
...
$this->data["listDetail"]=$this->data["product_details"];
			//	$this->data['getCommonFacilityDetails'] = $this->experience_model->getCommonFacilityDetails();
				//$this->data['getExtraFacilityDetails'] = $this->experience_model->getExtraFacilityDetails();
				//$this->data['getSpecialFeatureFacilityDetails'] = $this->experience_model->getSpecialFeatureFacilityDetails();
				//$this->data['getHomeSafetyFacilityDetails'] = $this->experience_model->getHomeSafetyFacilityDetails();
	...
	/*  View Experience ends */

	/*  individual experience status change  starts */
	...
	/* individual experience status change ends*/
	...
	/* featured experience ends */
	
	/* featured experienceType starts */
