	...
	public function insertAttribute(){
		if ($this->checkLogin('A') == ''){
			...
			if ($this->input->post('status') != ''){
			 ...
			 // $this->product_model->commonInsertUpdate(ATTRIBUTE,'insert',$excludeArr,$dataArr,$condition);
	...
	public function edit_attribute_form(){
		if ($this->checkLogin('A') == ''){
			...
			foreach ($this->data['attribute_details']->result() as  $key=>$val){
				$valArr =$val; // [name_en] => titEn
			}
			
			$this->data['valAre']=$valArr;
			
			//echo "<pre>";print_r($valArr);
	...
	public function insertEditListValue(){
		...
		if ($list_id == ''){
			...
			if ( $this->upload->do_upload('image')){
				...
				if ($lvID!=''){
				$this->setErrorMessage('error','File Should be JPEG,JPG,PNG and below 50*50 px');
				redirect('admin/attribute/edit_list_value_form/'.$lvID);
					
				}else{
					$this->setErrorMessage('error','File Should be JPEG,JPG,PNG and below 50*50 px');
					redirect('admin/attribute/add_list_value_form/');		
				}
