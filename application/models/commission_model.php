...
class Commission_model extends My_Model
{
  ...
  // echo $email;die;
   ...
   public function google_commission_login_details($email='')
   {
	...
	public function get_all_commission_tracking($sellerEmails){
		$Query = "select c.* , re.prd_id,re.currencycode,re.renter_id,re.cancelled,re.currencyPerUnitSeller,re.checkin,re.id as EnquiryId from ".COMMISSION_TRACKING." c JOIN ".RENTALENQUIRY." re on re.Bookingno=c.booking_no where  `host_email`='".$sellerEmails."'";
	...
	public function get_unpaid_commission_tracking($sellerEmail){
		...
		$Query = "select c.* , re.prd_id,re.currencycode,re.currencyPerUnitSeller,re.cancelled,re.checkin from ".COMMISSION_TRACKING." c JOIN ".RENTALENQUIRY." re on re.Bookingno=c.booking_no where  c.paid_status = 'no' AND  `host_email`='".$sellerEmail."'";
		return $this->ExecuteQuery($Query)->result_array();
	}
	...
	public function get_paid_details($sellerEmails)
    {
		...
		$Query = "select c.* , re.prd_id,re.currencyPerUnitSeller,re.currencycode,re.subtotal,re.cancelled,re.cancel_percentage from ".COMMISSION_TRACKING." c JOIN ".RENTALENQUIRY." re on re.Bookingno=c.booking_no where paid_status = 'yes' AND `host_email`='".$sellerEmails."'";
		return $this->ExecuteQuery($Query)->result_array();
  ...
  /*  public function get_all_rep_details()
   ...
   /*public function get_all_commission_tracking_rep($sellerEmail){
		$Query = "select c.* , re.prd_id, u.* from ".COMMISSION_TRACKING." c JOIN ".RENTALENQUIRY." re on re.Bookingno=c.booking_no JOIN ".USERS." u on u.email=c.host_email where  `host_email`='".$sellerEmail."'";
		return $this->ExecuteQuery($Query)->result_array();
	}*/
   ...
   public function get_all_commission_trackings($sellerEmail){
	...
	/*public function get_all_commission_tracking_reps($sellerEmail){
		$Query = "select c.* , re.prd_id, u.* from ".COMMISSION_REP_TRACKING." c JOIN ".RENTALENQUIRY." re on re.Bookingno=c.booking_no JOIN ".USERS." u on u.email=c.host_email where  `host_email`='".$sellerEmail."'";
		return $this->ExecuteQuery($Query)->result_array();
	}*/
