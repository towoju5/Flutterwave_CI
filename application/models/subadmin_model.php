...
class Subadmin_model extends My_Model
{
	...
	function get_rep_details_with_host(){
		$sql="select s.*,(select IFNULL(count(ur.id),0) from ".USERS." as ur where ur.rep_code= s.admin_rep_code) as host_count from ".SUBADMIN." as s, fc_users as u WHERE s.admin_rep_type='Representative' group by s.admin_rep_code order by s.id desc";
