...
class experience_order_model extends My_Model
{
	...
	public function view_order_details($status){
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.experience_title as product_name,pd.currency,pd.experience_id as PrdID,rq.Bookingno as bookingno,rq.currencycode,rq.currencyPerUnitSeller');
	...
	public function view_order_detailsexcel($status=''){
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.experience_title as product_name,pd.experience_id as PrdID,rq.currencycode,rq.Bookingno as bookingno,rq.currencyPerUnitSeller');
		$this->db->from(EXPERIENCE_BOOKING_PAYMENT.' as p');
		$this->db->join(USERS.' as u' , 'p.user_id = u.id');
		$this->db->join(EXPERIENCE.' as pd' , 'pd.experience_id = p.product_id');		
		 $this->db->join(EXPERIENCE_ENQUIRY.' as rq' , 'rq.id = p.EnquiryId');
	...
	$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.experience_title as product_name,pd.experience_id as PrdID,pd.experience_title as prd_name,pd.price');
...
/*********************************************** Payment Success Cart********************************************************/	
	...
	public function PaymentSuccess($userid='', $randomId='' ,$transId = '', $update_pay_type = ''){  //$payerMail
