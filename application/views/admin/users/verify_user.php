<style type="text/css">
	.table-pad td {padding:5px; text-align:center; vertical-align: middle;}
.table-pad td img
{
	height:100px;
	width: 100px;
								...
								<table border="1px" width="100%" class="table-pad" >
									...
									<tbody>
										...
										foreach ($userDetails->result() as $proof) {
											...
											?>
											<tr>
												<td><?php echo $proof_title;?><input type='hidden' name="proofID[]" value="<?php echo $proof->id; ?>"></td>
												<td>
													<?php 

													if(in_array($file_ext, $img_type)){
													?><a href='<?php echo ID_PROOF_PATH.$proof->proof_file;?>' target='_blank'>
														<img src="<?php echo ID_PROOF_PATH.$proof->proof_file; ?>" width='100' /></a>	
													<?php
												...
												<td>
													...
													<!--
														...
														<input type="radio" name='proof_status[<?php echo $proof->id; ?>]' id='verified_<?php echo $proof->id; ?>' value='V'  <?php if($proof->proof_status=='V') echo "checked"; ?> ></radio><b> Verified</b>
														</label>

														<label for="unverified_<?php echo $proof->id; ?>">
														<input type="radio" name='proof_status[<?php echo $proof->id; ?>]' id='unverified_<?php echo $proof->id; ?>' value='P' <?php if($proof->proof_status=='P') echo "checked"; ?> ></radio> <b> Unverified </b>
														</label>

														<label for="cancelled_<?php echo $proof->id; ?>">
														<input type="radio" name='proof_status[<?php echo $proof->id; ?>]' id='cancelled_<?php echo $proof->id; ?>' value="CL" <?php if($proof->proof_status=='CL') echo "checked"; ?> ></radio> <b> Cancelled</b>
														</label>
													</div>
												</td>
												<td><textarea name="comments[<?php echo $proof->id; ?>]" id="comments_<?php echo $proof->id; ?>"  rows='3'><?php echo $proof->proof_comments; ?></textarea></td>
											</tr>
											<?php
										}
