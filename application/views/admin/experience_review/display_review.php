				...
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						<tr>
							...
							<td class="center tr_select ">
                                <a href = "<?php echo base_url();?>view_experience/<?php echo $row['product_id'];?>" target="_blank"><?php echo ucfirst($row['product_title']);?></a>
							</td>
                             <td class="center tr_select ">
							...
							if ($allPrev == '1' || in_array('2', $Review)){
								...
								if ($mode == '0'){
							?>
								<a title="Click to inactive" class="tip_top" href="javascript:confirm_status('admin/experience_review/change_review_status/<?php echo $mode;?>/<?php echo $row['id'];?>');"><span class="badge_style b_done"><?php echo $row['status'];?></span></a>
							<?php
								}else {	
							?>
								<a title="Click to active" class="tip_top" href="javascript:confirm_status('admin/experience_review/change_review_status/<?php echo $mode;?>/<?php echo $row['id'];?>')"><span class="badge_style"><?php echo $row['status'];?></span></a>
							<?php 
								}
							}else {
							?>
							...
							<?php }?>
								<span><a class="action-icons c-suspend" href="admin/experience_review/view_review/<?php echo $row['id'];?>" title="View">View</a></span>
                                <?php if ($allPrev == '1' || in_array('3', $Review)){?>
							<span><a class="action-icons c-delete" href="javascript:confirm_delete('admin/experience_review/delete_review/<?php echo $row['id'];?>')" title="Delete">Delete</a></span><?php } ?>
							</td>
						</tr>
						<?php 
							$i=$i+1;}
