			...
			<div class="right_side overview schedule-experience form_container left_label listingInfo">
                    ...
                    <div class="dashboard_price_left">
                        ...
                        <h3><?php if($this->lang->line('experience_schedule') != '') { echo stripslashes($this->lang->line('experience_schedule')); } else echo " Set your timings ";?></h3>
			 ...
			 <!---List of schedule -->
				<div class="add_new_form">
					...
					<form onsubmit="return validate_form()" id="exp_schedule_form" name="exp_schedule_form" action="<?php echo base_url()."photos_listing/".$listDetail->row()->id;?>" method="post" accept-charset="UTF-8">
					...
					<div class="basic-next">			
                        ...
                        <div class="overview_title new_schedule_div" style="display:none;">
                        	...
                        	<h4 class="margin_top_20 margin_bottom_20"><label class="exp-reqlbl"><?php if($this->lang->line('experience_Schedule') != '') { echo stripslashes($this->lang->line('experience_Schedule')); } else echo "New Schedule";?> <span class="req"> *</span></label></h4>
                        ...
                        <?php } else{
                        	?>
	                            <div class="col-md-4 col-xs-12 no-padding margin_bottom_20">
									<label class="field_title service_det_edit" for="datetimepicker1">Choose date</label>
							...
							?>
									...
									<input type="hidden" class=" col-sm-2"  id="exp_currency" name="exp_currency"  value="<?php echo $listDetail->row()->currency;  ?>" readonly />
									<input type="hidden" class=" col-sm-2"  id="price_val" name="price"  value="<?php echo $listDetail->row()->price;  ?>" readonly />

								<div class="exp-addicon  col-md-4 col-xs-12 no-padding margin_top_10">
								<label style="margin-top: 15px;"></label>
								<button type="button" class="btn btn-sm btn-default" id='cancel_btn' title="cancel" onclick="cancel_dates_form()" >Cancel</span></button>
								<button type="button" class="btn btn-sm btn-success" id='add_btn' title="Add Date" onclick="add_dates()" >Submit</button></div>
...
<div class="exp-addicon"><button title="save" style='display:none;width: 40px;padding-right:4px;font-size: 16px;background: #752b7e; color:#ffffff;' type="button"  class="" id='save_timing_btn' onclick="save_timing()" >save</button></div> 
							...
							*/
		            		...
		            		<tr id="parent_<?php echo $row->id;?>">
			            		...
			            		<td>
									...
									<span class="" onclick="get_new_timesheet('<?php echo $row->id;?>','<?php echo $row->status;?>','<?php echo $allow; ?>');" title="Add Sheduled timings"><i class="fa fa-plus-square-o fa-lg cursor_pointer" aria-hidden="true"></i></span>&nbsp;&nbsp;
									...
									<span><a class="" href="admin/experience/delete_date/<?php echo $row->id;?>/<?php echo $row->experience_id;?>" title="Delete"><i class="fa fa-trash-o fa-lg cursor_pointer" aria-hidden="true"></i> </a></span>&nbsp;&nbsp;
									...
									?>
									<span class="" title="Make inactive"><a class="btn-sm" href='javascript:void(0);' id="status_<?php echo $row->id; ?>" onclick="change_date_status('<?php echo $row->id;?>',this);"><?php echo $active_str; ?> </a></span>&nbsp;&nbsp;
				...
				?>
			     <!-- Tour dates selection Ends  -->
					<div class="basic-next" id="next_button_div">
					 <button class="btn btn-success continue" type="button" onclick="next_form(event,'tagline_tab')"><?php if($this->lang->line('Continue') != '') { echo stripslashes($this->lang->line('Continue')); } else echo "Continue";?></button>
					 </div>
					<?php 
				}
					 ?>
...
function update_time_sheet(id,date_id){
	...
	var end_time = $('#time_sheet_'+id).find('input[name="end_time1"]').val(); 
	...
	if(hours_count!='' && hours_count>0){
		...
		{
			var diff = parseFloat(diff_start-diff_end); 
		}
		else
		{
			var diff = parseFloat(diff_end-diff_start); 
		}
...
if(startime_converted=='' || end_time=='' || schedule_date=='' || title=='' || description==''){
	...
	var e_time = $('#new_time_sheet_'+date_id).find('input[name="end_time1"]').val();
	...
	if(hours_count!='' && hours_count>0){
		...
		var tm = parseInt(12);
		var diff_start = s_time.replace(":", ".");
		var ent = e_time.replace(":", "."); 
			ent = ent.slice(0, -2); 

		e_timeampm = e_time.replace(/[^a-z]/gi, '');
		if(e_timeampm == 'pm' && parseInt(ent) > 12)
		{
			var diff_en = e_time.replace(":", "."); 
			var diff_end = diff_en.slice(0, -2);
		}
		else if(e_timeampm == 'pm' && parseInt(ent) < 12)
		{
			var diff_en = e_time.replace(":", "."); 
			diff_en = diff_en.slice(0, -2);
			var diff_end = parseInt(diff_en) + parseInt(tm);
		}
		else
		{
			var diff_en = e_time.replace(":", "."); 
...
function assign_to_time_hour(date_id,tim='')
{
	...
	if(hours_count!='' && hours_count!=null && hours_count>0){
		...
		if(parseInt(end_val)<24){
			//end_val=end_val.replace(".", ":");
			$('#end_time_'+date_id).val(objectToString);
		}else{
			$('#end_time_'+date_id).val('');
