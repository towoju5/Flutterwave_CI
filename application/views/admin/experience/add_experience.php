...
<?php
error_reporting(1);
//print_r($product_details->row());
$imageUpload = $this->uri->segment(5,0);
$this->load->view('admin/templates/header.php');
//print_r($product_details->row());
 /*
foreach ($product_details->result() as $product_listing){
...
<script type="text/javascript">
      	function save_tab1(){
			...
			if(pro_id!='' && pro_id!='0'){
					...
					data:{'user_id':user_id,'experience_title':product_title,'pro_id':pro_id,'date_count':date_count,'type_id':type_id,'price':price,'security_deposit':security_deposit,'cancellation_policy':cancellation_policy,'status':status,'currency':currency},
					dataType:'json',
					success:function(json)
					{
...
<script src="js/site/addExperience.js"></script>
    ...
    <div class="grid_12">
      <div class="widget_wrap">
        ...
        <div class="widget_content">        
					...
					<form class = 'form_container left_label listingInfo' id = 'addproduct_form1111', enctype = 'multipart/form-data' action="admin/experience/UpdateExperience" onkeypress="return event.keyCode != 13;" method="POST" accept-charset="UTF-8">
					<?php /*
					<input type='hidden' name="currency" id='currency' value='<?php if(!empty($product_details)) echo $product_details->row()->currency; else echo $admin_currency_code; ?>'> */ ?>
          ...
          <div id="tab1">
            <ul class="tab-areas1">
                ...
                <div class="form_grid_12">
                  <label class="field_title" for="user_id">Experience Owner Name <span class="req">*</span></label>
              ...
              <li>
                <div class="form_grid_12">
                  ...
                  <?php if(!empty($product_details)){  $Valid = trim(stripslashes($product_details->row()->id)); } else {  $Valid=0;}?>
                    <input name="product_title" id="product_title" type="text" tabindex="1" class="required large tipTop" title="Please enter the Property name"   onkeypress="//return runScript(event);"  onkeydown = " return (event.keyCode!=13);"
                    value="<?php if(!empty($product_details)){ echo trim(stripslashes($product_details->row()->product_title)); }?>"/><span id="product_title_error" style="color:#f00;display:none;"> Special Characters are not allowed!</span>
               ...
               <li>
                <div class="form_grid_12">
                  <label class="field_title" for="user_id">Experience Category <span class="req">*</span></label>
                  <div class="form_input">
                  	<?php if(empty($product_details)) { ?>
              		<select style="" class="other-opt" onblur="affectDateCount(this.value)" onchange="affectDateCount(this.value)" id="home_type_new" name="experience_category" required>
                     ...
                     <?php } else { ?>
                      <span class=""><?php if($product_details->row()->date_count>1) echo "Immersions"; else echo "Experience"; ?></span>
                     <?php } ?>
                  </div>
                </div>
              </li>
                  ...
                  <div class="form_input">
                  	<select class="gends" id="type_id" name="type_id">
						...
						foreach ($experienceTypeList->result() as $type) { ?>
							<option value="<?php echo $type->id; ?>" <?php if(!empty($product_details)){ if(trim($type->id) == trim($product_details->row()->type_id)) echo 'selected="selected"'; } ?> ><?php echo ucfirst($type->experience_title); ?></option>
							<?php
						}
						?>
              ...
              <li>
	                <div class="form_grid_12">
	                  <label class="field_title" for="date_count">Date Count <span class="req">*</span></label>
	                  <div class="form_input">
					  	<label class="" for="date_count"><span class="req"></span></label>
					  		<?php if(empty($product_details)) { ?>
	                    	   <input class="select-bor" style="width:50%" type="number" min="1" name='date_count' id='date_count' placeholder="Enter Date Count" onchange="javascript:experienceAdminDetailview(this,document.getElementById('prdiii').value,'date_count');" />
                             	 
	                        <span id="date_count_error" style="color:red;"></span>
	                        <?php } else {?>
	                          <span class=""><?php echo $product_details->row()->date_count;  ?></span>
	                           <input class="select-bor" style="width:50%" type="hidden" min="1" name='date_count' id='date_count' value='<?php echo $product_details->row()->date_count;  ?>' placeholder="Enter Date Count" />
	                        <?php } ?>
	                  </div>
	                ...
	                <div class="form_grid_12">
					  ...
					  <label class="" for="price"><span class="req"></span></label>
	                    <input type="text" onkeypress="return isNumber(event)" name="price" id="price" tabindex="9" class="required large tipTop" title="Please enter the property price" value="<?php if(!empty($product_details)){
							...
							} ?>"  onchange="javascript:PriceInsert(this.value,document.getElementById('prdiii').value,'price');"/><span id="price_error_valid" style="color:#f00;display:none;"> Only Numbers are not allowed!</span>
			   ...
			   <li>
                <div class="form_grid_12">
                  ...
                  <div class="form_input">
				  <label class="" for="price"><span class="req"></span></label>
                  <input type="text" class="required large tipTop" name="security_deposit" title="Please enter the deposit amount" id="security_deposit" style="  width: 379px;" value="<?php if(!empty($product_details)){ 
	                  	...
	                  	} ?>"><span id="security_deposit_error_valid" style="color:#f00;display:none;"> Only Numbers are not allowed!</span>
			  ...
			  <li>
                <div class="form_grid_12">
                  ...
                  <select class="gends" id="cancellation_policy" name="cancellation_policy">
						...
						<option value="Flexible" <?php if(!empty($product_details)){ if($product_details->row()->cancel_policy=='Flexible'){echo 'selected="selected"';}}?>>Flexible</option>
						<option value="Moderate" <?php if(!empty($product_details)){ if($product_details->row()->cancel_policy=='Moderate'){echo 'selected="selected"';}}?>>Moderate</option>
					<option value="Strict" <?php if(!empty($product_details)){ if($product_details->row()->cancel_policy=='Strict'){echo 'selected="selected"';}}?>>Strict</option>
					</select>
			...
			<div id="tab3">
				...
				<ul class="1tab-areas3">
					...
					<li>
                        ...
                        <input type="hidden" class=" col-sm-2"  id="experience_id" name="experience_id" value="<?php echo $product_details->row()->experience_id ; ?>"/>
                        <input type="hidden" class=" col-sm-2"  id="dates_id" name="dates_id" />

                        <div class="form_grid_12">
							<label class="field_title" style="width: 30%;" for="user_id">Schedule <span class="req">*</span></label>
							<div class="form_input">
								<?php if($product_details->row()->date_count>1) { ?>
		                            ...
		                            <div class="col-md-4 col-xs-12 no-padding">
										<label class="field_title service_det_edit" for="datetimepicker1">From Date: </label>
										<input type="text" class="dev_tour_date col-sm-2"  id="datetimepicker1"  onclick="datepick();" name="from_date" />
									</div>

									<div class=" col-md-4 col-xs-12 no-padding">
										<label class="field_title service_det_edit" for="to_date">To Date: </label>
										<input type="text" class=" col-sm-2"  id="to_date" name="to_date" readonly value=""/>
									</div>
									<!--
									<div class=" col-md-4 col-xs-12 no-padding">
								...
								<ul>
									<li>
										<div class="form_grid_12">
											...
											<div class="form_input">
												<?php if($product_details->row()->date_count>1) { ?>
													<input type="text" class="dev_multi_schedule_date col-sm-2"  id="schedule_date1" onkeypress="return isNumber(event)" name="schedule_date[]" onclick="setDatepickerHere();" />
												<?php } else { ?>
													<input type="text" class="dev_tour_date dev_schedule_date col-sm-2"  id="schedule_date1" name="schedule_date[]" readonly />
												<?php	
												}?>	
											</div>
										</div>
									...
									<li>	
										<div class="form_grid_12">
											...
											<div class="form_input">
												<textarea class="" required name="schedule_description[]" ></textarea>
											</div>
										</div>
									</li>
									<li class="exp-button">

										<button type="button" class="btn_small" id='add_timing_btn_1' onclick="add_timing_row(1)" ><i class="fa fa-plus-circle" aria-hidden="true"></i>Add New</button>
									</li>
								</ul>	
						</li>
						...
						<li class="exp-button"><button  style='display:  none; clear: both;' type="button" class="btn_small" id='save_timing_btn' onclick="save_timing()"  >Save Timing</button></li>
						<!--- Experiece Panel End-->

				<li class="she-add-date"><button type="button" class="btn_small" id='add_btn' title="Add Date" onclick="add_dates()" ><i class="fa fa-plus-circle" aria-hidden="true"></i>Add Date</button></li>

						<li class="exp-button">
							 <input type="button" class="filter-btn btn_small" id="update_btn" style="display: none" name="" value="Update Date" onclick="update_tab2()">
							<input type="reset" class="filter-btn btn_small" id="reset_btn" style="display: none;"  name="" value="Cancel" onclick="reset_reload()">
              ...
              <li>
                <div class="form_grid_12">
                  <div class="form_input exp-button" style="margin:0px;width:100%;">
                    <input type="button" class="btn_small btn_blue prvTab" tabindex="9" value="Previous"/>
                    <input type="button" class="btn_small btn_blue nxtTab" tabindex="9" value="Next"/>
					<input type="button" class="btn_small btn_blue" value="Save" onclick="save_tab4();" />
                  </div>
                </div>
              </li>
            </ul>
          ...
          <div id="tab5">
            <ul>
              	...
              	<h3>Experience Details</h3><p>A description of your experience displayed on your public listing page. </p>
	            <li>
	                <div class="form_grid_12">
	                  	<label class="field_title" for="space">The Experience Description</label>
	                  	<div class="form_input">
	                        <textarea name="experience_description" id="experience_description" tabindex="13" style="width:370px;height:100px;" class="large tipTop" title="what makes  your listing unique ?" ><?php if(!empty($product_details)){ echo trim(stripslashes($product_details->row()->experience_description)); }?></textarea>
	                    </div>  
	            ...
	            <li>
	                <div class="form_grid_12">
	                  	<label class="field_title" for="short_description">The Experience subtitle</label>
	                  	<div class="form_input">
	                        <textarea name="short_description" id="short_description" tabindex="13" style="width:370px;height:100px;" class="large tipTop" title="what makes  your listing unique ?"><?php if(!empty($product_details)){ echo trim(stripslashes($product_details->row()->short_description)); }?></textarea>
	                    </div>  
	            ...
	            <li>
	                <div class="form_grid_12">
	                  	<label class="field_title" for="location_description">The Location Description</label>
	                  	<div class="form_input">
	                        <textarea name= "location_description" id = "location_description" tabindex="13" style="width:370px;height:100px;" class="large tipTop" title="what makes  your listing unique ?"><?php if(!empty($product_details)){ echo trim(stripslashes($product_details->row()->location_description)); }?></textarea>
	                    </div>  
              	...
              	<li>
              		...
              		<span class="my-edit-lbl" for="emailaddress"><h3><?php  echo "Language";?></h3></span>
				...
				<li>
                    ...
                    <h3 style="padding: 0 9px; margin-top: 0px;">Guide Provides</h3>
                        ...
                        <div class="form_grid_12">
                            <label class="field_title" for="detailed_title">Detailed Title:<span class="req"></span></label>
              				<div class="form_input">
                               
                                <input type="text" class=" col-sm-2"  id="detailed_title" name="detailed_title" value=""/>
                        </div> 
                </li><li>       
                        <div class="form_grid_12">
                            <label class="field_title service_det_edit" for="kit_count">Count: </label>
              				<div class="form_input">
                                <input type="text" class=" col-sm-2"  id="kit_count" name="kit_count" value=""/>
                        	</div>
                        </div> 
                 </li><li>        
                        <div class="form_grid_12">
                            <label class="field_title service_det_edit" for="kit_description">description </label>
              				<div class="form_input">
                                <textarea class="title_overview" id="kit_description" placeholder="Description the kit contains" rows="8"  name="kit_description" id="kit_description" style="color:#000 !important;min-width:100%;max-width:100%;"></textarea>
                        	</div>
                        </div> 
                 </li><li>           	
                        ...
                        <div class="exp-button">
                        <button type="button" class="next_button btn_small" id='add_kit_btn' onclick="add_kit()" style="width:100px;">Add </button>
                        <input type="button" class="filter-btn" id="update_kit_btn" style="display: none;" name="" value="Update" onclick="update_kit_tab2()">
                        <input type="reset" class="filter-btn" id="reset_kit_btn" style="display: none;" name="" value="Cancel" onclick="reset_reload()">
                        </div>
	                            ...
	                            <tr>
	                                ...
	                                <td>
	                                    ...
	                                    <?php // if($product_details->row()->status == '0') {
	                                     ...
	                                     ?>    
	                                        <span class="action-icons c-edit" onclick="get_kit_data('<?php echo $row->id; ?>','<?php echo $row->kit_title; ?>','<?php echo $row->kit_detailed_title; ?>','<?php echo $row->kit_count; ?>','<?php echo $descrpt; ?>');" title="Edit" style="cursor: pointer;"><?php if($this->lang->line('back_Edit') != '') { echo stripslashes($this->lang->line('back_Edit')); } else echo "<i class='fa fa-pencil-square-o' aria-hidden='true'></i>"; ?></span> |
	                                        
	                                        
	                                         <span><a class="action-icons c-delete" href="admin/experience/delete_kit_package/<?php echo $row->id;?>/<?php echo $row->product_id;?>" title="Delete"><i class="fa fa-trash-o delete-icon fa-lg" aria-hidden="true"></i> </a></span>
	                                     <?php // } ?>
	                                       <!--  <span><a class="action-icons c-delete" onclick="javascript:delete_season_data('<?php //echo $row->season_id;?>','<?php //echo $row->product_id;?>','<?php //echo $row->date_from; ?>','<?php //echo $row->date_to; ?>');" title="Delete">Delete</a></span> -->
       	...
       	<div id="tab6">
            <ul>
				...
				<h3>Guest & Guide Details </h3><p> </p>
				<li>
					<div class="form_grid_12">
						<label class="field_title" for="confirm_email">About Group Size  </label>
						<div class="form_input">
							<textarea name="group_size" id="space" tabindex="13" style="width:370px;height:100px;" class="large tipTop" title="what makes  your listing unique ?"><?php if(!empty($product_details)){ echo trim(stripslashes(ucfirst($product_details->row()->group_size))); }?></textarea>
				...
				<li>
					<div class="form_grid_12">
						...
						<div class="form_input">
							<textarea name="guest_requirement" id="guest_requirement" tabindex="13" style="width:370px;height:100px;" class="large tipTop" title="what makes  your listing unique ?"><?php if(!empty($product_details)){ echo trim(stripslashes(ucfirst($product_details->row()->guest_requirement))); }?></textarea>
						</div>
					</div>
				</li>
            	...
            	<ul>
	              	<li>
		                <div class="form_grid_12">
		                  ...
		                  <div class="form_input">
		                    <input name="meta_title" id="meta_title" type="text" tabindex="1" class="required large tipTop" title="Please enter the page meta title" value="<?php if(!empty($product_details)){ echo trim(stripslashes($product_details->row()->meta_title)); }?>"/>
		                  </div>
		                </div>
	              	</li>
	               	...
	               	<li>
	                    ...
	                    <textarea name="meta_keyword" id="meta_keywords" tabindex="13" style="width:370px;height:150px;" class="required large tipTop" title="Please enter the keywords"><?php if(!empty($product_details)){ echo trim(stripslashes($product_details->row()->meta_keyword)); }?></textarea>
						</div>
						</div>
					</li>
	                <li>
						<div class="form_grid_12">
						<label class="field_title" for="description">Meta Description<span class="req">*</span></label>
						<div class="form_input">
						<textarea name="meta_description" id="meta_description" tabindex="13" style="width:370px;height:150px;" class="required large tipTop" title="Please enter the meta description"> <?php if(!empty($product_details)){ echo trim(stripslashes($product_details->row()->meta_description)); }?></textarea>
						</div>
						</div>
					</li>
...
function save_timing() {
    ...
    if(error==0 && error_in_mandatory==0 && error_time==0 && error_time_des==0)
    {
	   $.ajax({
	    ...
	    }else if(error_time>=0){ alert('Start time should be less than end time.');}
     }
		...
		$.ajax({
			...
			success:function(data)
			{
				...
				if(data.case == 1){
					...
					schedule_title 			= $("input[name='schedule_title[]']").map(function(){return $(this).val();}).get();
					schedule_description 	= $("input[name='schedule_description[]']").map(function(){return $(this).val();}).get();
					error =  error_in_mandatory = error_time = error_time_des = 0;
				   // alert(schedule_description); 
				    for (i=0;i<start_time.length;i++) {
				    	if(schedule_title[i]=='' )
				...
				else
				{
					if(pro_id!='' && pro_id!='0'){
						$.ajax({
							...
							data:{'pro_id':pro_id,'edit_pro_id':edit_pro_id,'experience_description':experience_description,'short_description':short_description,'location_description':location_description
							},
							dataType:'json',
							success:function(json){
	         	...
	         	var edit_pro_id = $('#edit_pro_id').val();
          		...
          		if(pro_id!='' && pro_id!='0'){
          			$.ajax({
						...
						data:{'pro_id':pro_id,'edit_pro_id':edit_pro_id,'space':space,'guest_requirement':guest_requirement
						},
						dataType:'json',
						success:function(json){
