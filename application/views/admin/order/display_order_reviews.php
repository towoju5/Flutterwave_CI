	...
	<div id="content">
		...
		?>
			<div class="review_top">
                ...
                <p class="fl"><span class="r_left fl">Checkin Date : </span><span class="fl"><?php echo date("d-m-Y",strtotime($order_details->row()->checkin));?></span></p>
                <p class="fl"><span class="r_left fl">Checkout Date : </span><span class="fl"><?php echo date("d-m-Y",strtotime( $order_details->row()->checkout));?></span></p>
...
<!-- 			<hr class="review_hr"> -->
		...
		?>
			<table width="100%" border="0" cellspacing="0" cellpadding="0" style="float:left;border:1px solid #cecece;">
		        <tbody>
			        <tr bgcolor="#f3f3f3">
			        	<td width="14%" style="border-right:1px solid #cecece; text-align:center;"><span style="font-size:12px; font-family:Arial, Helvetica, sans-serif; font-weight:bold; color:#000000; line-height:38px; text-align:center;">Property Image</span></td>
			            <td width="30%" style="border-right:1px solid #cecece;text-align:center;"><span style="font-size:12px; font-family:Arial, Helvetica, sans-serif; font-weight:bold; color:#000000; line-height:38px; text-align:center;">Property Name</span></td>
			            ...
			            <td width="8%" style="border-right:1px solid #cecece;text-align:center;"><span style="font-size:12px; font-family:Arial, Helvetica, sans-serif; font-weight:bold; color:#000000; line-height:38px; text-align:center;">No of guest</span></td>
						...
						<?php 
								$base =base_url();
								$url=getimagesize($base.'server/php/rental/'.$imgArr[0]);
								if(!is_array($url))
								{
								  $img="1"; //no
								}
						...
						<?php if (count($imgArr)>0 && $img=='0') { ?>
							<img src="<?php if(strpos($prodImg, 's3.amazonaws.com') > 1)echo $prodImg;else echo base_url()."server/php/rental/".$prodImg; ?>" alt="<?php echo $prod_details[$orderRow->product_id]->row()->product_title;?>" width="70">
						<?php } else { ?>
							<img src="<?php if(strpos($prodImg, 's3.amazonaws.com') > 1)echo $prodImg;else echo base_url()."server/php/rental/dummyProductImage.jpg" ?>" alt="<?php echo $prod_details[$orderRow->product_id]->row()->product_title;?>" width="70">
						<?php } ?>
			            ...
			            <td style="border-right:1px solid #cecece;text-align:center;border-top:1px solid #cecece;"><span style="font-size:13px; font-family:Arial, Helvetica, sans-serif; font-weight:normal; color:#000000; line-height:30px;  text-align:center;"><?php echo $orderRow->NoofGuest;?></span></td>
						...
						/* if($admin_currency_code!=$row->currency_code){
								//echo convertCurrency($row->currency_code,$admin_currency_code,$row->amount);
								echo customised_currency_conversion($currencyPerUnitSeller,$row->amount);
							 }else
							 echo $row->amount;
						 */
...
<!-- 	                        	<option <?php if ($orderRow->received_status == 'Need refund'){echo 'selected="selected"';}?> value="Need refund"><?php if($this->lang->line('order_need_refund') != '') { echo stripslashes($this->lang->line('order_need_refund')); } else echo "Need refund"; ?></option>
				...
				<p class="fr"><span class="r_left fl">Sub Total : </span><span class="fl"><?php echo $admin_currency_symbol;
					$subtotal = $orderRow->subTotal - $orderRow->host_fee; 
					if($admin_currency_code != $orderRow->currency_code ){
						//echo convertCurrency($orderRow->currency_code,$admin_currency_code,$orderRow->subTotal);	
						echo customised_currency_conversion($currencyPerUnitSeller,$subtotal);
					}else{	echo $subtotal; }
				...
				<!--  Tax Amount 
				<div style="clear: both;"></div>
				<p class="fr"><span class="r_left fl">Tax Amount : </span><span class="fl"><?php
			/* 	echo $admin_currency_symbol;
				...
				<div style="clear: both;"></div>
					<p class="fr"><span class="r_left fl">Host Fee [To Admin]: </span><span class="fl"><?php echo $admin_currency_symbol;
					if($admin_currency_code != $orderRow->currency_code ){
						//echo convertCurrency($orderRow->currency_code,$admin_currency_code,$result);	
						echo customised_currency_conversion($currencyPerUnitSeller,$orderRow->host_fee);
					}else{	echo $orderRow->host_fee; }
					//echo 	$result;?></span></p>
				...
				<div style="clear: both;"></div><br>
					<p class="fr" style="width:100%;text-align:right;"><span class="r_left" style="clear:both;">Grand Total : </span>
						...
						if($admin_currency_code != $orderRow->currency_code ){
							...
							echo customised_currency_conversion($currencyPerUnitSeller,$total_amount);
						}else{	echo  $total_amount; }
	        ...
	        <div class="review_comments" style="float: left;">
	        	 ...
	        	 <section class="comments comments-list comments-list-new">
		        	 ...
		        	 ?>
		        		 <li class="comment" style="position: relative;padding: 17px 0 12px 43px;z-index: 1;min-height: 20px;">
							...
							<p style="font-size: 10px;font-style:italic;color:green;"><?php echo "On ". date("d-m-Y H:i" , strtotime($cmt_row->date)) ; //echo $cmt_time;?></p>
						</li>
					<?php 
		        	 	}
		        	 }
				...
				<div style="margin:20px 0;float:left;">
					<form action="javascript:void(0)" onsubmit="post_order_comment_admin('<?php echo $orderRow->product_id;?>','<?php echo $order_details->row()->dealCodeNumber;?>')" method="post">
	                    <textarea cols="40" rows="7" class="text order_comment_<?php echo $orderRow->product_id;?>" name="comments" placeholder="<?php if($this->lang->line('header_write_comment') != '') { echo stripslashes($this->lang->line('header_write_comment')); } else echo "Write a comment"; ?>..." id="comments"></textarea><br />
