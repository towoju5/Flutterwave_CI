					...
					<div class="widget_content">
						...
						<tr>
							...
							<th class="tip_top" title="Click to sort">
								Status							</th>
								<th class="tip_top" title="Click to sort">
								Rep Code							</th>								<th class="tip_top" title="Click to sort">								Added By							</th>
							<th width="12%">
								 Action							</th>
						</tr>
						</thead>
						...
						<tr>
							...
							<?php							            $this->db->reconnect();				                        $this->db->select('*');				                        $this->db->from(SUBADMIN);				                        $this->db->where('admin_rep_code',$row->rep_code);										$result_u = $this->db->get();							?>							<td class="center"><?php 
							if($row->rep_code!='')
							{
								echo $row->rep_code;								echo '<br>';								echo '(';								echo $result_u->row()->name;								echo ')';
							}
							else
							{
								echo 'Not available';
							}
							?> </td>							<td class="center"><?php 							if($row->rep_code!='')							{								echo 'Representative';															}							else							{								echo 'Website';							}							?> </td>
<!-- 							<td class="center">
							<?php 
							if ($allPrev == '1' || in_array('2', $Host)){
...
<?php							if ($allPrev == '1' || in_array('1', $Host)){?>
							...
							<?php }if ($allPrev == '1' || in_array('3', $Host)){?>	
								<span><a class="action-icons c-tick" href="javascript:confirm_delete('admin/seller/update_seller/<?php echo $row->id;?>')" title="Delete">Delete</a></span>
							<?php }?>
							</td>
						</tr>
						<?php 
