...
jQuery(function($) {
	$('#stripe-pay-button').click(function(event) {
		...
		if(terms == false)
		{
			$("#terms_warn").html('<?php if($this->lang->line('Term of service should be acceptable') != '') { echo stripslashes($this->lang->line('Term of service should be acceptable')); } else echo "Term of service should be acceptable";?>');
			return false;
		}
		else if(terms == true)
		{
                ...
                <div class="payment_box">
					...
					<div id="TabbedPanels1" class="TabbedPanels">
						...
						<div class="TabbedPanelsContentGroup">
							<form method="post" action="site/product/HostPaymentCredit" id="credit_card_forms_stripe" accept-charset="UTF-8">
							
							<input type="hidden" name="payment_method" id="payment_method" value="StripeCreditCard">
							...
							<input type="hidden" name="hosting_priceAdminCur" id="hosting_priceAdminCur" value="<?php echo $hosting_price; ?>">
							
							
							<input type="hidden" name="hosting_price" id="hosting_price" value="<?php
		                    if($product->currency!="USD"){
		                    	echo convertCurrency($product->currency,"USD",$hosting_price);
		                    }
		                    else{
		                     	echo $hosting_price;
		                    }
...
<p>Your Price for this listing is  <?php echo $currencySymbol . " ".$hostPrice; ?> </p>
									...
									<div  class="payment_right">
										...
										<div style="position:relative;" class="pypg_lb">
											...
											<input  id="cardNumber" maxlength="16" name="cardNumber" type="text" style="width:100%; margin-top:5px;" autocomplete="off" data-stripe="number"/>
										</div>
										...
										<input type="hidden" id="productPrice_id" value="<?php echo $product->price; ?>" name="productPrice"/>
							...
							<?php if($this->lang->line('This payment transacts in') != '') { echo stripslashes($this->lang->line('This payment transacts in')); } else echo "This payment transacts in";?><?php echo " ".$currencySymbol; ?> <?php
				...
				<!-- start - currency validation in prd_currency-->
				<?php //if ($product->currency!=$this->session->userdata('
				//')) { ?>
						<!--<span style="color:red">Please Change Currency into <?php // echo  $product->currency; ?>--> </span>
				<?php //} ?>
				<!-- end - currency validation in prd_currency-->
				
				<!--<div style="<?php // if ($product->currency!=$this->session->userdata('currency_type')) { echo "display:none"; } ?>">-->
	
				<div class="payment_agree" style="padding-bottom:10px;">
					<p>
					<input type="hidden" value="<?php echo $hosting_price;?>" id="totpricePaypal"> 
					...
					<input type="submit" style="margin:10px 0 0 10px;" id="stripe-pay-button" value="<?php if($this->lang->line('Book it using Credit Card') != '') { echo stripslashes($this->lang->line('Book it using Credit Card')); } else echo "Book it using Credit Card";?>"  class="btn large green"></span>
					<div id="paypal_but"><input type="submit" id="paypal" style="display:none;margin:10px 0 0 10px;" onclick="paypal_form();" value="<?php if($this->lang->line('Book it using Paypal') != '') { echo stripslashes($this->lang->line('Book it using Paypal')); } else echo "Book it using Paypal";?>"  class="btn large green"></div>
				</div>
	...
	<div class="dashboard_bottom">
		<div class="main">
			<ul class="dashboard_links">
				...
				<li class="center">
					<div class="trust_box trust_shadow"><img src="images/host_guarantee.png" width="75" height="72" /></div>
...
{ 
	...
	if($('#phone_no').val()==''){
	...
	else
		{
			...
			$('#terms_warn').html('<p style="color:#F00; margin-right:3px;"><?php if($this->lang->line('This_field_is_required') != '') { echo stripslashes($this->lang->line('This_field_is_required')); } else echo "This field is required";?></p> ');
...
function paypal_form()
{
	...
	var amount = $('#totpricePaypal').val(); 
	var productAmount = $('#productPrice_id').val(); 
